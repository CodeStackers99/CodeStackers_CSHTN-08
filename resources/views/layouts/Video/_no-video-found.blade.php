<div class=" section d-flex flex-row justify-content-center align-items-center section-divider">
    <div class="text">
        <h2><strong class="text-hblack text-capitalize text-center sub-heading">No Videos found :(</strong></h2>
        @auth
            @if (auth()->user()->isAdmin() || auth()->user()->isTeacher())
                <div class="button text-center">
                    <a
                        href="{{ route('courses.subcourses.playlists.videos.create', [$course->slug, $subCourse->slug, $playlist->slug]) }}"
                        class="styled-btn styled-rounded text-muted border border-dark p-1" style="text-decoration:none">
                        <span class="styled-button-text">Create Video</span>
                    </a>
                </div>
            @endif
        @endauth
